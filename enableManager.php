...
$DB->insert_record('hierarchy_ismanager', array('userid' => $userid,'active' => '1'));
$DB->insert_record('hierarchy_managerlog', array('userid' => $userid,'adminid' => $USER->id,'action' => 'Manager Status Enabled'));
    echo $OUTPUT->header();
 
    echo '<div class="manageTable" style=" width:400px;">
  <h2 style="font-size:1.1em; color:#999; line-height:20px; text-transform:uppercase; word-wrap:break-word;">'.get_string('enabledmanager', 'block_hierarchy').'</h2>
    <br>'.get_string('managerenablednotification', 'block_hierarchy').'
    <div class="editClose"> <a href="../../user/profile.php?id='.$_GET['id'].'">'.get_string('continue', 'block_hierarchy').'</a></div></div>';
      echo '<div class="manageTable" style="width:400px; ">
  <h2 style="font-size:1.1em; color:#999; line-height:20px; text-transform:uppercase; word-wrap:break-word;">'.get_string('log', 'block_hierarchy').':</h2>';
