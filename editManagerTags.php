...
$PAGE->set_url('/blocks/hierarchy/editManagerTags.php');
...
<h2 style="font-size:1.1em; color:#999; line-height:20px; text-transform:uppercase; word-wrap:break-word;">This area is intended to permit temporary access (until the nightly reset). This can be useful for testing manager tags. In order to permanently use these please ensure you use the profile fields.</h2>
<br>
<div class="editClose"> <a href="manageCategories.php">'.get_string('continue', 'block_hierarchy').'</a></div></div>';

echo'<form action="saveManagerTags.php" method="POST"><input type="hidden" name="userid" value="'.$userid.'"></input>';

$getcategory = "SELECT * FROM {hierarchy_category} ORDER BY name";
$catResults = $DB->get_records_sql($getcategory);
//$currentTags = 'SELECT `tagid` as id FROM {hierarchy_managertags} WHERE `userid`='.$userid;
//$currentResults = $DB->get_records_sql($currentTags);

foreach ($catResults as $catResult) {
  echo '<div class="select"><a class="title">'.$catResult->name.'</a><br><select multiple style="width:200px; height:240px;" name="'.$catResult->id.'[]"><option value = "">None</option>';

  $getTags = 'SELECT tt.`id` AS id, tt.`name` AS `name`, tu.`userid` AS `user`  FROM {hierarchy_tags} tt LEFT JOIN {hierarchy_managertags} tu ON tu.`tagid`=tt.`id` AND tu.`userid`='.$userid.'
